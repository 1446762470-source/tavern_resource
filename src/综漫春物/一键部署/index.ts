(async () => {
  const BASE = 'http://localhost:6622/综漫春物/';
  const WB_URL = BASE + '综漫春物剧情版.json';
  ...
  try {
    ...
    await createOrReplaceWorldbook('综漫春物剧情版', wbEntries, { render: 'immediate' });
    try { await rebindCharWorldbooks('current', { primary: '综漫春物剧情版', additional: [] }); } catch (_) {}
  ...
  try {
    await updateScriptTreesWith(trees => {
      ...
      trees.push({ type: 'script' as any, enabled: true, name: '浮窗手机', id: 'float-phone-' + Date.now(), content: `(function(){var s=document.createElement('script');s.src='${BASE}脚本/浮窗手机/loader.js';s.onerror=function(){window.parent.toastr.error('浮窗手机加载失败')};document.head.appendChild(s)})();`, info: '综漫春物', button: { enabled: false, buttons: [] }, data: {}, export_with: { data: false, button: false } } as any);
      return trees;
    }, { type: 'character' });
  ...
  window.parent.toastr.success('综漫春物 部署完成！', '✅');
})();
